...
FOCUS_DELAY_SEC = 1.2
USB_SCAN_DEVICES = [USB_DEVICE, 0, 1, 2, 3, 4, 5, 16, 17, 18]
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            ...
            set_step(f"Outer Box {outer_index}/{outer_total} : USB camera. Press CAPTURE to run YOLO capture")
            set_status("CAM3 Ready. Press CAPTURE CAM3")
            # CAM3 จะเปิดจริงตอนกด CAPTURE เพื่อให้ capture_direct_or_yolo ใช้ logic เดิมของ T8
            preview_label.config(image="", text="CAM3 USB Preview will start after CAPTURE")
...
def capture_cam3_stage():
    ...
    try:
        ...
        set_status(f"CAM3 opening /dev/video{dev}...")
        ...
        if not result:
            set_status("CAM3 capture cancelled or failed", RED)
            return
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Manual Capture - Dataset Mode")
...
action_button(capture_left, "RESET CURRENT CAMERA", GRAY, fg=TEXT, command=reset_current_camera, width=760, height=52)
