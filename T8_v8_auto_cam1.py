...
# =========================================================
# CAM1 AUTO MOTION DETECT CONFIG
# ใช้ค่า threshold จาก picam_motion_threshold_test.py
# =========================================================
AUTO_CAM1_CAPTURE = True
MOTION_THRESHOLD = 8_000_000
STABLE_TIME = 1.5
MIN_AREA = 25_000
DIFF_THRESHOLD = 30
BLUR_SIZE = 21
AUTO_CAM1_START_DELAY_MS = 500
...
class ManualPiCamera:
    ...
    def make_gray(self, frame_rgb):
        gray = cv2.cvtColor(frame_rgb, cv2.COLOR_RGB2GRAY)
        gray = cv2.GaussianBlur(gray, (BLUR_SIZE, BLUR_SIZE), 0)
        return gray

    def calc_motion(self, background_gray, frame_rgb):
        gray = self.make_gray(frame_rgb)
        ...
        return gray, motion_score, int(max_area)

    def wait_object_by_motion(self, alive_cb=None):
        """CAM1 auto detect object by background difference.
        Returns True when motion/object is stable for STABLE_TIME.
        """
        if self.picam2 is None:
            raise RuntimeError(f"{self.name} not opened")

        alive_cb = alive_cb or (lambda: True)

        set_status(f"{self.name}: preparing background...")
        time.sleep(0.8)

        frame = self.picam2.capture_array()
        background_gray = self.make_gray(frame)

        motion_start = None
        last_log = 0

        set_status(f"{self.name}: waiting object by threshold...")
        while alive_cb() and self.picam2 is not None:
            frame = self.picam2.capture_array()
            self.latest_frame = frame

            gray, score, area = self.calc_motion(background_gray, frame)
            ...
            if score >= MOTION_THRESHOLD and area >= MIN_AREA:
                ...
                set_status(f"{self.name}: object detected {elapsed:.1f}/{STABLE_TIME:.1f}s | SCORE={score} AREA={area}")

                if elapsed >= STABLE_TIME:
                    set_status(f"{self.name}: object stable. Auto capture ready.")
                    return True
            else:
                motion_start = None
                if now - last_log >= 0.5:
                    set_status(f"{self.name}: waiting object | SCORE={score} AREA={area}")
                    last_log = now
    ...
    def autofocus(self, wait_sec=None):
        ...
        wait_sec = FOCUS_DELAY_SEC if wait_sec is None else wait_sec
        af_done = False
    ...
    def capture_file(self, path):
        ...
        frame = self.picam2.capture_array()
        self.latest_frame = frame
        # frame from Picamera2 is RGB, cv2.imwrite needs BGR
        bgr = cv2.cvtColor(frame, cv2.COLOR_RGB2BGR)
        cv2.imwrite(path, bgr, [cv2.IMWRITE_JPEG_QUALITY, 100])
        return path
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            ...
            current_camera = cam
            cam.start_preview()
            if AUTO_CAM1_CAPTURE:
                set_status("CAM1 Ready. Auto waiting object by threshold")
                root.after(AUTO_CAM1_START_DELAY_MS, capture_current)
            else:
                set_status("CAM1 Ready. Press CAPTURE - CAM2 will auto capture")
            ...
            current_camera = cam
            cam.start_preview()
            set_status("CAM2 Ready. Auto capture running")

        elif current_stage == "cam3":
            set_step(f"Outer Box {outer_index}/{outer_total} : USB camera preview. YOLO capture will run automatically")
            ...
            current_camera = cam
            cam.start_preview()
            set_status(f"CAM3 Ready on /dev/video{dev}. Auto capture will start")
...
def reset_current_camera():
    """
    T8_v8: Reset only the current camera inside the same Python process.

    Flow:
    - stop preview thread
    - close current camera object
    - close CAM3 worker object if any
    - force garbage collection
    - wait longer for Picamera2/libcamera/V4L2 to release resources
    - reopen the same current stage camera

    This does NOT reload OCR / YOLO models because the app process is not restarted.
    """
    global auto_cam3_capture_on_ready, current_camera
    ...
    set_status("Reset current camera: stop thread / close / cleanup...")

    # 1) Stop preview loop first so capture_array/read loop releases the camera cleanly.
    try:
        preview_stop.set()
    except Exception as e:
        print("preview_stop set error:", e)

    # 2) Give preview thread time to exit before closing camera.
    try:
        cam_ref = current_camera
        th = getattr(cam_ref, "thread", None)
        if th is not None and th.is_alive():
            th.join(timeout=1.5)
    except Exception as e:
        print("preview thread join error:", e)

    # 3) Close current camera object.
    try:
        if current_camera is not None:
            current_camera.close()
    except Exception as e:
        print("current_camera close error:", e)
    current_camera = None

    # 4) Close CAM3 YOLO worker holder if it exists.
    try:
        cam = current_cam_holder.get("cam")
        if cam is not None:
            cam.close()
    except Exception as e:
        print("current_cam_holder close error:", e)
    current_cam_holder["cam"] = None

    # 5) Force cleanup object references. This helps Picamera2/libcamera release resources.
    try:
        gc.collect()
    except Exception as e:
        print("gc.collect error:", e)

    # 6) Wait longer than the old 0.8s so OS/libcamera/V4L2 can release the camera.
    time.sleep(2.0)

    # 7) Reopen same stage camera without restarting app or reloading models.
    set_status("Reset current camera: reopen same stage...")
...
# =========================================================
def start_job():
    global running, quantity, outer_total, inner_index, outer_index, current_stage, auto_cam3_capture_on_ready
...
def capture_pi_stage():
    """
    T8_v8 Auto CAM1 Flow:
    - CAM1 waits for object by motion threshold.
    - When object is stable for STABLE_TIME, program captures CAM1 automatically.
    - Then opens CAM2 and captures automatically.
    ...
    # ===================== CAM1 AUTO MOTION DETECT + CAPTURE =====================
    try:
        if current_camera is None:
            raise RuntimeError("CAM1 is not opened")

        if AUTO_CAM1_CAPTURE:
            detected = current_camera.wait_object_by_motion(
                lambda: running and current_stage == "cam1" and not preview_stop.is_set()
            )
            if not detected:
                set_status("CAM1 auto detect stopped or reset", RED)
                return

        set_status(f"CAM1 autofocus {FOCUS_DELAY_SEC:.1f}s...")
        current_camera.autofocus(FOCUS_DELAY_SEC)
        set_status("CAM1 auto capturing...")
        current_camera.capture_file(cam1_path)
        set_status(f"Saved CAM1: {cam1_path}", GREEN)
    except Exception as e:
        set_status(f"CAM1 capture error: {e}", RED)
        root.after(0, lambda e=e: messagebox.showerror("CAM1 Error", str(e)))
        return

    # Close CAM1 before opening CAM2.
    stop_preview_loop()
    safe_close_camera()
    time.sleep(0.25)
    ...
    try:
        ...
        preview_stop.clear()
        last_preview_time = 0
        cam2 = ManualPiCamera(PICAM2_INDEX, "CAM2")
        cam2.open()
        current_camera = cam2
        cam2.start_preview()

        set_status(f"CAM2 autofocus {FOCUS_DELAY_SEC:.1f}s...")
        cam2.autofocus(FOCUS_DELAY_SEC)
        set_status("CAM2 auto capturing...")
        cam2.capture_file(cam2_path)
        set_status(f"Saved CAM2: {cam2_path}", GREEN)
    except Exception as e:
    ...
    root.after(0, lambda n=completed_no: show_capture_complete_overlay("✓ COMPLETE", f"Inner Box {n} / {quantity}", 1000))
    time.sleep(1.05)
...
def capture_cam3_stage():
    ...
    # A short delay prevents V4L2 / OpenCV from keeping the old stream handle alive.
    stop_preview_loop()
    safe_close_camera()
    time.sleep(0.8)
    ...
    try:
        ...
        root.after(0, lambda n=completed_outer: show_capture_complete_overlay("✓ COMPLETE", f"Outer Box {n} / {outer_total}", 1000))
        time.sleep(1.05)
    except Exception as e:
        try:
            if cam3 is not None:
...
def reset_to_input():
    global running, current_stage, quantity, outer_total, inner_index, outer_index, current_job_dir, auto_cam3_capture_on_ready
    running = False
    auto_cam3_capture_on_ready = False
    stop_preview_loop()
    safe_close_camera()
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Manual Capture - Dataset Mode T8_v8 Auto CAM1")
...
tk.Label(input_card, text="Auto CAM1 Dataset Capture", font=("Arial", 22, "bold"), bg=CARD, fg=TEXT).pack(anchor="w", pady=(0, 10))
