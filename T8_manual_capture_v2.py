            ...
            while not preview_stop.is_set() and self.cap is not None:
                try:
                    ...
                    if not ok or frame is None:
                        bad_count += 1
                        if bad_count >= 10:
                            root.after(0, lambda: set_status(f"{self.name}: Cannot Read Frame", RED))
                            break
                        time.sleep(0.08)
                        continue
                    bad_count = 0
...
def reset_current_camera():
    ...
    if current_stage not in ("cam1", "cam2", "cam3"):
        return
    set_status("Reset current camera... Please reconnect if needed.")
    open_current_stage_camera()


def _video_name_map():
    """Return {device_number: device_name} from v4l2-ctl when available."""
    mapping = {}
    try:
...
def find_usb_device():
    """
    Find a readable USB camera. Important: after unplug/replug, Rapoo may move
    from /dev/video0 to /dev/video1, while /dev/video18 can be metadata and cannot read frames.
    """
    name_map = _video_name_map()
    candidates = []

    # Prefer devices whose v4l2 name looks like a USB/Rapoo camera.
    for dev, name in name_map.items():
        lname = str(name).lower()
        if "rapoo" in lname or "usb" in lname or "camera" in lname:
            candidates.append(dev)

    # Then try config/manual list.
    for dev in USB_SCAN_DEVICES:
        candidates.append(dev)

    # Then try every /dev/video*.
    ...
    for dev in candidates:
        ...
        # Skip common metadata/non-capture nodes when a paired USB camera exposes 2 nodes.
        if dev >= 18 and dev != USB_DEVICE:
            continue

        if _can_read_usb_device(dev):
            print(f"CAM3 selected /dev/video{dev} ({name_map.get(dev, '')})")
            return dev
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Manual Capture - Dataset Mode V2")
