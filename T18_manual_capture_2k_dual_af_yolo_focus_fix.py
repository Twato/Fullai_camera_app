...
class ManualPiCamera:
    ...
    def autofocus(self, wait_sec=None):
        """Trigger autofocus for exactly the configured settle time.

        T18 behavior:
        - Start AF trigger/cycle without blocking when possible.
        - Wait only wait_sec total.
        - Do not call autofocus_cycle(wait=True) and then wait again.
        """
        if self.picam2 is None:
            raise RuntimeError(f"{self.name} not opened")

        wait_sec = float(self.focus_delay_sec if wait_sec is None else wait_sec)
        t0 = time.time()
        af_started = False

        # Prefer non-blocking AF trigger so the configured delay is the total wait.
        ...
        # Fallback: try autofocus_cycle without waiting.
        if not af_started:
            try:
                if controls is not None:
                ...
                print(f"{self.name} autofocus_cycle non-blocking not available/failed:", e)

        # Final fallback: if no non-blocking method is available, use blocking AF once.
        # Do not wait another full wait_sec after it.
        if not af_started:
            try:
                if controls is not None:
                    self.picam2.set_controls({"AfMode": controls.AfModeEnum.Auto})
                ...
                print(f"{self.name} blocking autofocus failed:", e)

        remaining = wait_sec - (time.time() - t0)
        end_time = time.time() + max(0.0, remaining)
        while time.time() < end_time:
            if preview_stop.is_set():
                break
            time.sleep(0.02)

        print(f"{self.name} AF total wait: {time.time() - t0:.2f}s (target {wait_sec:.2f}s)")
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            set_step(f"Inner Box {inner_index}/{quantity} : CAM1 preview. Press CAPTURE once, CAM1+CAM2 will autofocus/capture together")
            set_status("CAM1 opening/standby...")
            cam = get_or_open_persistent_picam(1)
            cam.ensure_preview_mode()
            current_camera = cam
            cam.start_preview()
            set_status(f"CAM1 Ready. No-switch 2K stream / dual AF ready {PI_CAPTURE_WIDTH}x{PI_CAPTURE_HEIGHT}")
...
def capture_pi_stage():
    """
    T18 Flow:
    - Based on T15 no-switch 2K stream for CAM1/CAM2.
    - Operator presses CAPTURE only on CAM1 page.
    - CAM1 and CAM2 autofocus at the same time.
    - CAM1 and CAM2 capture at almost the same time.
    - Images are queued to background save threads.
    - Then the flow moves to the next Inner Box or CAM3.
    """
    global current_stage, inner_index, running, auto_cam3_capture_on_ready, current_camera, last_preview_time

    if current_stage != "cam1":
        set_status("Please capture from CAM1. CAM1/CAM2 will run together.", YELLOW)
        return
    ...
    try:
        set_status("Preparing CAM1 + CAM2 for parallel autofocus/capture...", BLUE)
        ...
        # Do NOT call stop_preview_loop() before autofocus, because the T15 autofocus wait loop
        # checks preview_stop and would skip its settle wait if preview_stop is already set.
        try:
            cam1.capturing = True
            cam2.capturing = True
        except Exception:
        ...
        root.after(0, lambda: set_step(f"Inner Box {inner_index}/{quantity} : CAM1 + CAM2 autofocus/capture together"))
        set_status(
            f"CAM1 + CAM2 timed autofocus together... CAM1 {PICAM1_FOCUS_DELAY_SEC:.1f}s / CAM2 {PICAM2_FOCUS_DELAY_SEC:.1f}s",
            BLUE,
        )
        ...
        def af_and_capture(cam, delay_sec, path, label):
            try:
                t0 = time.time()
                print(f"{label}: parallel autofocus start")
                cam.autofocus(delay_sec)
                t1 = time.time()
                print(f"{label}: parallel autofocus done in {t1 - t0:.2f}s")
                print(f"{label}: parallel capture start")
                cam.capture_file(path, return_to_preview=False)
                t2 = time.time()
                print(f"{label}: parallel capture queued in {t2 - t1:.2f}s")
                results[label] = path
            except Exception as e:
                errors.append(f"{label}: {e}")

        t_all = time.time()
        th1 = threading.Thread(target=af_and_capture, args=(cam1, PICAM1_FOCUS_DELAY_SEC, cam1_path, "CAM1"), daemon=True)
        th2 = threading.Thread(target=af_and_capture, args=(cam2, PICAM2_FOCUS_DELAY_SEC, cam2_path, "CAM2"), daemon=True)
        th1.start()
        th2.start()
        th1.join()
        th2.join()
        print(f"CAM1+CAM2 T18 timed parallel AF/capture total: {time.time() - t_all:.2f}s")
        ...
        try:
            ...
            pass
        set_status(f"CAM1/CAM2 parallel capture error: {e}", RED)
