...
# =========================================================
APP_VERSION = "T24_SYSTEM_LOGGING"
...
try:
    ...
    print("T24 Pi camera threshold applied:",
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T24_SYSTEM_LOGGING",
...
def process_product(product_no):
    ...
    while running:
        ...
        current_camera = cam1
        try:
            log_event("CAMERA_OPEN_START", camera="CAM1", index=PICAM1_INDEX)
            cam1.open()
            log_event("CAMERA_OPEN_OK", camera="CAM1", index=PICAM1_INDEX)
            ...
            log_event("CAPTURE_DONE", camera="CAM1", path=cam1_path, duration_sec=round(time.time() - cap_start, 3))
            add_process_image(cam1_path, "CAM1_VENDOR_TOA_FULL")
        except Exception as e:
            log_event("CAM1_ERROR", level="ERROR", error=str(e), traceback=traceback.format_exc())
        ...
        current_camera = cam2
        try:
            log_event("CAMERA_OPEN_START", camera="CAM2", index=PICAM2_INDEX)
            cam2.open()
            log_event("CAMERA_OPEN_OK", camera="CAM2", index=PICAM2_INDEX)
            ...
            log_event("CAPTURE_DONE", camera="CAM2", path=cam2_path, duration_sec=round(time.time() - cap_start, 3))
            add_process_image(cam2_path, "CAM2_TOB_FULL")
        except Exception as e:
            log_event("CAM2_ERROR", level="ERROR", error=str(e), traceback=traceback.format_exc())
            if not is_retry_action():
...
def process_toc(toc_no=1, toc_total=1):
    ...
    while running:
        ...
        try:
            log_event("CAMERA_OPEN_START", camera="CAM3", device=USB_DEVICE)
            cam3.open()
            log_event("CAMERA_OPEN_OK", camera="CAM3", device=USB_DEVICE)
            ...
            log_event("CAM3_ERROR", level="ERROR", error=str(e), traceback=traceback.format_exc())
            if not is_retry_action():
        ...
        log_event("CAM3_CAPTURE_RESULT", duration_sec=round(time.time() - yolo_capture_start, 3), detection_count=len(capture_result.get("detections", [])), image_path=capture_result.get("image_path", ""))
...
def schedule_storage_cleanup():
    ...
    def run_cleanup_worker():
        ...
        try:
            print("=" * 60)
            print("T24 DAILY STORAGE CLEANUP START", now_iso())
            deleted = auto_delete_old_storage()
            print(f"T24 DAILY STORAGE CLEANUP DONE. Deleted {len(deleted)} day folder(s).")
            print("=" * 60)
            try:
            ...
            print("T24 DAILY STORAGE CLEANUP ERROR:", e)
            try:
...
def startup_preload_worker():
    ...
    set_preload_status("LOADING", "Preloading YOLO model if camera_test supports it...", yolo="RUN")
    try:
        yolo_msg = preload_yolo_model_if_available()
        set_preload_status(detail=f"YOLO preload: {yolo_msg}", yolo="OK")
    except Exception as e:
...
root.title("AI Camera Station - YOLO ROI OCR - T24")
