...
def finalize_attempt_status(status, reason=""):
    ...
    if job_data:
        ...
        job_data["job_finished_at"] = datetime.now().isoformat(timespec="seconds") if status != "RUNNING" else ""
        if reason:
            if status == "CANCELLED":
...
def add_process_image(path, label):
    if not current_process_attempt_meta or not path:
        return
    current_process_attempt_meta.setdefault("images", []).append({"label": label, "path": path})
    try:
...
def build_inner_box_record(inner_no, cam1_data, cam2_data, process_dir=""):
    ...
    if process_dir:
        process_dir = relocate_process_attempt_to_parent(process_dir, inner_folder)
...
def process_product(product_no):
    ...
    while running:
        clear_action()
        pending_inner_ref = f"INNER_{product_no:03d}_VENDOR_PENDING"
        process_dir = next_process_attempt_dir("inner", pending_inner_ref)
