...
inner_toa_lot_by_index = {}  # Test memory: Inner Box no -> TOA Lot No OCR value
...
def make_product_result_section(parent, key, title, icon_key, field1, value1, field2, value2):
    ...
    tk.Label(title_row, text=title, bg=CARD, fg=TEXT, font=("Arial", 18, "bold")).pack(side="left")
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    # Test memory for Outer Box validation: TOC1-TOC6 compare against TOA Lot No values.
    # In this current test UI, TOA Lot No uses cam1_text as the mocked OCR value.
    ...
    # Temporary validation UI for testing only.
    # Later these values should come from real DB / duplicate / date-code validation.
    vendor_box_status = mock_read_status(cam1_text)
    toa_lot_status = mock_read_status(cam1_text)
    tob_lot_status = mock_read_status(cam2_text)

    date_values = [cam1_text, cam1_text, cam2_text]
    vendor_date_validation, vendor_date_status = mock_date_compare_validation(cam1_text, date_values)
    toa_date_validation, toa_date_status = mock_date_compare_validation(cam1_text, date_values)
    tob_date_validation, tob_date_status = mock_date_compare_validation(cam2_text, date_values)

    update_product_result_section(
        "vendor",
        cam1_roi,
        cam1_text,
        mock_vendor_db_validation(cam1_text),
        vendor_box_status,
        cam1_roi,
        cam1_text,
        vendor_date_validation,
        vendor_date_status,
    )
    ...
    update_product_result_section(
        ...
        cam1_roi,
        cam1_text,
        mock_duplicate_validation(cam1_text),
        toa_lot_status,
        cam1_roi,
        cam1_text,
        toa_date_validation,
    ...
    update_product_result_section(
        ...
        cam2_roi,
        cam2_text,
        mock_duplicate_validation(cam2_text),
        tob_lot_status,
        cam2_roi,
        cam2_text,
        tob_date_validation,
...
def reset_to_input():
    global running, current_index, current_outer_index, delivery_no, en_no, quantity, outer_total, inner_toa_lot_by_index
...
def title_with_icon(parent, icon_key, text, bg=CARD):
    ...
    tk.Label(frame, text=text, bg=bg, fg=TEXT, font=("Arial", 17, "bold")).pack(side="left")
    return frame
...
def add_action_button(parent, text, bg, fg="white", command=None, width_px=205, height_px=44, frame_bg=None):
...
input_card = tk.Frame(page_input, bg=CARD, padx=70, pady=58, highlightbackground=BORDER, highlightthickness=1)
input_card.place(relx=0.5, rely=0.50, anchor="center", width=820, height=640)

tk.Label(input_card, text="Start Test Job", font=("Arial", 30, "bold"), bg=CARD, fg=TEXT).pack(anchor="w", pady=(0, 26))


def make_input(parent, title):
    tk.Label(parent, text=title, bg=CARD, fg=TEXT, font=("Arial", 16, "bold")).pack(anchor="w", pady=(0, 7))
...
tk.Button(input_card, text="START TEST", font=("Arial", 18, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job).pack(fill="x", ipady=13, pady=(8, 16))
tk.Button(input_card, text="EXIT APP", font=("Arial", 18, "bold"), bg=RED, fg="white", activebackground=RED_DARK, activeforeground="white", relief="flat", command=exit_app).pack(fill="x", ipady=13)
...
add_action_button(header_buttons, "Restart The Process", bg="#bfc3c7", fg=TEXT, command=restart_process, width_px=205, height_px=44)
add_action_button(header_buttons, "Cancel Job", bg=RED, fg="white", command=cancel_job, width_px=205, height_px=44)
add_action_button(header_buttons, "OK / NEXT", bg="#27ae60", fg="white", command=on_result_ok, width_px=205, height_px=44)
...
def header_info_inline(parent, title, value_width=18):
    ...
    tk.Label(box, text=f"{title} :", bg=CARD, fg=MUTED, font=("Arial", 12, "bold"), anchor="w").pack(side="left")
    val = tk.Label(box, text="-", bg=CARD, fg=TEXT, font=("Arial", 16, "bold"), width=value_width, anchor="w")
...
def header_info_box(parent, title, value_width=16):
    ...
    tk.Label(box, text=title, bg=CARD, fg=MUTED, font=("Arial", 11, "bold"), anchor="w").pack(anchor="w")
    val = tk.Label(box, text="-", bg=CARD, fg=TEXT, font=("Arial", 16, "bold"), width=value_width, anchor="w")
...
info_box_title = tk.Label(inner_box_container, text="Inner Box", bg=CARD, fg=MUTED, font=("Arial", 11, "bold"), anchor="w")
info_box_title.pack(anchor="w")
info_box = tk.Label(inner_box_container, text="-", bg=CARD, fg=TEXT, font=("Arial", 16, "bold"), width=14, anchor="w")
...
progress_title_label = tk.Label(product_header, text="Inner Box", bg=CARD, fg=TEXT, font=("Arial", 12, "bold"))
progress_title_label.pack(side="left")

product_row = tk.Frame(capture_right, bg=CARD)
product_row.pack(fill="x")
progress_label = tk.Label(product_row, text="0 / 0", bg=CARD, fg=TEXT, font=("Arial", 12, "bold"))
progress_label.pack(side="left")
progress_percent_label = tk.Label(product_row, text="0%", bg=CARD, fg=TEXT, font=("Arial", 12, "bold"))
...
tk.Label(check_header, text="Checklist", bg=CARD, fg=TEXT, font=("Arial", 12, "bold")).pack(side="left")
...
def make_check_item(name):
    item = tk.Label(checklist_frame, text=f"[ ]   {name}", bg=SOFT, fg=TEXT, font=("Arial", 14, "bold"), anchor="w", padx=14, pady=11, relief="flat")
...
tk.Label(step_header, text="Step", bg=SOFT, fg="#34495e", font=("Arial", 11, "bold")).pack(side="left")

step_label = tk.Label(step_status_box, text="-", bg=SOFT, fg=TEXT, font=("Arial", 12), wraplength=280, justify="left")
...
tk.Label(status_detail_header, text="Status", bg=SOFT, fg="#34495e", font=("Arial", 11, "bold")).pack(side="left")

status_label = tk.Label(step_status_box, text="-", bg=SOFT, fg=TEXT, font=("Arial", 12), wraplength=280, justify="left")
...
result_status_badge = tk.Label(result_left_header, text="WAIT REVIEW", bg="#f1c40f", fg=TEXT, font=("Arial", 14, "bold"), padx=16, pady=8)
...
cancel_result_btn = add_action_button(result_button_frame, "Cancel Job", bg=RED, fg="white", command=cancel_job, width_px=205, height_px=44, frame_bg=BG)
restart_result_btn = add_action_button(result_button_frame, "Restart The Process", bg="#bfc3c7", fg=TEXT, command=restart_process, width_px=205, height_px=44, frame_bg=BG)
ok_next_btn = add_action_button(result_button_frame, "OK / NEXT", bg="#27ae60", fg="white", command=on_result_ok, width_px=205, height_px=44, frame_bg=BG)
