...
# =========================================================
APP_VERSION = "T28_T21_CAMERA_OPTIMIZED_FULLAPP"
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T28_T21_CAMERA_OPTIMIZED_FULLAPP",
...
class OptimizedPiCamera:
    ...
    def wait_object(self, background, keep_running):
        ...
        while keep_running():
            ...
            score = int(cv2.countNonZero(th)) + int(area)
            last_score = score
            ...
            if score >= PICAM_MOTION_THRESHOLD:
                ...
                self._status(f"{self.name}: Object detected {elapsed:.1f}/{PICAM_STABLE_TIME:.1f}s | score={score}")
                if elapsed >= PICAM_STABLE_TIME:
                    return True
            else:
                stable_start = None
                self._status(f"{self.name}: Waiting object | score={score}")
...
root.title("AI Camera Station - YOLO ROI OCR - T28 T21 Camera Optimized")
