...
def find_usb_device():
    """
    หา USB Camera จาก v4l2-ctl --list-devices เท่านั้นเป็นหลัก
    เพื่อไม่ไปเปิด device ของ Pi เช่น /dev/video20-35 ที่ทำให้ select() timeout / ค้าง

    หลักการ:
    - เลือกเฉพาะกลุ่มที่ชื่อเป็น Rapoo/USB camera
    - ใช้ /dev/video เลขต่ำสุดของกลุ่มนั้นก่อน เช่น /dev/video0 หรือ /dev/video1
    - ไม่ลอง scan pispbe/rp1-cfe/rpi-hevc-dec
    - ไม่ลอง /dev/video18 ถ้ามี /dev/video0 หรือ /dev/video1 เพราะ 18 มักเป็น metadata node
    """
    ...
    # fallback เฉพาะ manual list เลขต่ำ ๆ เท่านั้น ไม่ glob ทั้งระบบ
    ...
    for dev, name, source in candidates:
        ...
        seen.add(dev)
        if dev >= 18:
            # ลอง high node เฉพาะกรณีไม่มี low node อ่านได้จริง ๆ แต่ยังไม่ข้ามในรอบนี้ถ้าเป็น candidate สุดท้าย
            pass
...
def maybe_auto_capture_cam3():
    """หลัง CAM1/CAM2 ครบ ให้ CAM3 เริ่ม YOLO capture อัตโนมัติ ไม่ต้องกด CAPTURE อีกครั้ง"""
    global auto_cam3_capture_on_ready
    ...
    # หน่วงนิดหน่อยให้ preview / device settle หลังเปิดกล้อง
...
def capture_pi_stage():
    ...
    - Program captures CAM1, then opens CAM2 and captures automatically.
    ...
    # ===================== CAM1 MANUAL CAPTURE =====================
    try:
        set_status(f"CAM1 focusing {FOCUS_DELAY_SEC:.1f}s...")
    ...
    try:
        ...
        set_status(f"CAM2 focusing {FOCUS_DELAY_SEC:.1f}s...")
        time.sleep(FOCUS_DELAY_SEC)
...
def capture_cam3_stage():
    ...
    # ห้ามเปิดทดสอบ /dev/video เดิมซ้ำในขณะที่ preview ยังเปิดอยู่
    # เพราะ OpenCV/V4L2 บน Pi บางครั้งจะทำให้ stream ค้างหรือ timeout
    # ถ้า preview มี latest_frame แปลว่า device นี้อ่านได้แล้ว จึง release แล้วให้ YOLO เปิดต่อ
    try:
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Manual Capture - Dataset Mode V7 HMI")
...
input_card.place(relx=0.5, rely=0.50, anchor="center", relwidth=0.85, relheight=0.70)
...
tk.Button(input_card, text="START MANUAL CAPTURE", font=("Arial", 16, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job).pack(fill="x", ipady=10, pady=(4, 10))
tk.Button(input_card, text="EXIT APP", font=("Arial", 16, "bold"), bg=RED, fg="white", activebackground=RED_DARK, activeforeground="white", relief="flat", command=exit_app).pack(fill="x", ipady=9)
