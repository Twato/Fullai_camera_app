...
# หลัง CAM0 + CAM1 ถ่ายครบ 1 กล่อง ระบบจะรอให้พื้นที่ CAM0 กลับมาใกล้เคียง background เดิม
REMOVE_BOX_EMPTY_THRESHOLD = 50000
REMOVE_BOX_STABLE_TIME = 2.0
...
def run_picamera(camera_index, camera_name):
    global current_camera, last_picam0_background
...
def wait_remove_box_and_ready(inner_no, total_inner):
    ...
    - แจ้งให้เอากล่องออก
    - เปิด CAM0 ดูภาพเบา ๆ
    - เทียบภาพปัจจุบันกับ background ล่าสุดของ CAM0
    - ถ้าภาพกลับมาใกล้เคียง background และนิ่งครบ REMOVE_BOX_STABLE_TIME
      ถือว่าพร้อมสำหรับกล่องถัดไป
    """
    global current_camera
    ...
    try:
        ...
        # ถ้าไม่มี background อ้างอิง ให้รอสั้น ๆ แล้วไปต่อ
        ...
        set_status("Status: Checking empty background with CAM0")
        ...
        while running:
            ...
            print(f"REMOVE BOX CHECK SCORE: {score}")

            if score < REMOVE_BOX_EMPTY_THRESHOLD:
                if stable_start is None:
                    stable_start = time()

                stable_time = time() - stable_start
                set_status(
                    f"Status: Background Empty Stable "
                ...
                stable_start = None
                set_status(f"Status: Waiting Remove Box / Score={score}")
