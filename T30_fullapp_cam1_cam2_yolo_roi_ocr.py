...
# =========================================================
APP_VERSION = "T30_CAM1_CAM2_YOLO_ROI_OCR"
...
CAM1_INNER_DETECTION_MODEL_PATH = "/home/toto/AI_CAMERA_TEST_YOLO_ROI/Models/TOA_model/Models_detection_VNTOA_V1.pt"
CAM2_INNER_DETECTION_MODEL_PATH = "/home/toto/AI_CAMERA_TEST_YOLO_ROI/Models/TOB_model/Models_detection_TOB_V1.pt"
...
INNER_FIELD_ALIASES = {
    "vendor_box_id": [
        ...
        "vender_box", "box_id", "boxid", "box-id", "vn_box", "vn_box_id", "vendor"
    ],
    "vendor_date_code": [
        "vendor_date_code", "vendor_date", "vender_date_code", "vender_date",
        "vn_date", "date_vendor", "date_vender", "datecode_vendor"
    ],
    "toa_lot_no": [
        "toa_lot_no", "toa_lot", "lot_toa", "toa_lotno", "toa_lot_no", "toa"
    ],
    "toa_date_code": [
        "toa_date_code", "toa_date", "date_toa", "datecode_toa"
    ],
    "tob_lot_no": [
        "tob_lot_no", "tob_lot", "lot_tob", "tob_lotno", "tob"
    ],
    "tob_date_code": [
        "tob_date_code", "tob_date", "date_tob", "datecode_tob"
    ],
}
...
def map_inner_class_to_field(class_name, camera_key="cam1"):
    """Map YOLO class name to the final data field used by validation/storage."""
    n = _normalize_label_name(class_name)
    for field, aliases in INNER_FIELD_ALIASES.items():
        for alias in aliases:
            a = _normalize_label_name(alias)
            if n == a or a in n:
                # Avoid mapping plain date to the wrong camera if class name is too broad.
    ...
    with inner_detection_lock:
        if camera_key == "cam1":
            if cam1_inner_detection_model is None:
                ...
                log_event("INNER_YOLO_LOAD_START", camera="CAM1", path=CAM1_INNER_DETECTION_MODEL_PATH)
                cam1_inner_detection_model = YOLO(CAM1_INNER_DETECTION_MODEL_PATH)
        ...
        if camera_key == "cam2":
            if cam2_inner_detection_model is None:
                from ultralytics import YOLO
                log_event("INNER_YOLO_LOAD_START", camera="CAM2", path=CAM2_INNER_DETECTION_MODEL_PATH)
                cam2_inner_detection_model = YOLO(CAM2_INNER_DETECTION_MODEL_PATH)
...
def yolo_detect_inner_rois(image_path, camera_key):
    """Run inner-label YOLO detection and return detection dicts compatible with crop_by_box()."""
...
def process_inner_yolo_roi_ocr(image_path, camera_key):
    ...
    # Safe fallback when class names are not mapped yet.
    # This keeps the app usable while we inspect class names in logs.
    fallback = None
    if not fields:
        fallback = process_center_roi_ocr(image_path, f"{camera_key}_center_fallback")
        if camera_key == "cam1":
            fields.setdefault("vendor_box_id", fallback)
            fields.setdefault("toa_lot_no", fallback)
            fields.setdefault("vendor_date_code", fallback)
            fields.setdefault("toa_date_code", fallback)
        else:
            fields.setdefault("tob_lot_no", fallback)
            fields.setdefault("tob_date_code", fallback)

    # Additional fallback for partial models: if CAM1 has only vendor/toa classes or CAM2 has one tob class.
    if camera_key == "cam1":
        first_any = next(iter(fields.values()), None) or (unmapped[0] if unmapped else None)
        if first_any is not None:
            fields.setdefault("vendor_box_id", first_any)
            fields.setdefault("vendor_date_code", fields.get("vendor_box_id", first_any))
            fields.setdefault("toa_lot_no", fields.get("toa_lot_no", first_any))
            fields.setdefault("toa_date_code", fields.get("toa_lot_no", first_any))
    elif camera_key == "cam2":
        first_any = next(iter(fields.values()), None) or (unmapped[0] if unmapped else None)
        if first_any is not None:
            fields.setdefault("tob_lot_no", first_any)
            fields.setdefault("tob_date_code", fields.get("tob_lot_no", first_any))
...
def get_inner_field_data(data, field_key, fallback=None):
    ...
    if isinstance(fields, dict) and fields.get(field_key):
        return fields.get(field_key) or {}
    return fallback if fallback is not None else (data if isinstance(data, dict) else {})
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T30_CAM1_CAM2_YOLO_ROI_OCR",
...
def build_inner_box_record(inner_no, cam1_data, cam2_data, process_dir=""):
    ...
    vendor_box_data = get_inner_field_data(cam1_data, "vendor_box_id", cam1_data)
    vendor_date_data = get_inner_field_data(cam1_data, "vendor_date_code", vendor_box_data)
    toa_lot_data = get_inner_field_data(cam1_data, "toa_lot_no", cam1_data)
    toa_date_data = get_inner_field_data(cam1_data, "toa_date_code", toa_lot_data)
    tob_lot_data = get_inner_field_data(cam2_data, "tob_lot_no", cam2_data)
    tob_date_data = get_inner_field_data(cam2_data, "tob_date_code", tob_lot_data)
...
def make_validation_status_badge(parent, status):
    status = (status or "WAIT").upper()
    if status == "PASS":
        text, bg, fg = "เนยโ€ PASS", GREEN, "white"
    elif status == "NG":
        text, bg, fg = "เนยโ€” NG", RED, "white"
    else:
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    vendor_box_roi = get_inner_field_data(cam1_data, "vendor_box_id", cam1_data).get("roi_path", "")
    vendor_date_roi = get_inner_field_data(cam1_data, "vendor_date_code", get_inner_field_data(cam1_data, "vendor_box_id", cam1_data)).get("roi_path", "")
    toa_lot_roi = get_inner_field_data(cam1_data, "toa_lot_no", cam1_data).get("roi_path", "")
    toa_date_roi = get_inner_field_data(cam1_data, "toa_date_code", get_inner_field_data(cam1_data, "toa_lot_no", cam1_data)).get("roi_path", "")
    tob_lot_roi = get_inner_field_data(cam2_data, "tob_lot_no", cam2_data).get("roi_path", "")
    tob_date_roi = get_inner_field_data(cam2_data, "tob_date_code", get_inner_field_data(cam2_data, "tob_lot_no", cam2_data)).get("roi_path", "")
...
def start_job():
    ...
    clear_action()
    init_job_data()
    log_event("JOB_START", en_no=en_no, delivery_no=delivery_no, quantity=quantity, outer_total=outer_total)
...
root.title("AI Camera Station - YOLO ROI OCR - T29 Threshold Fix")
