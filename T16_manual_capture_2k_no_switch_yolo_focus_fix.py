...
# T16 experimental no-switch mode for Pi CAM1/CAM2.
...
class ManualPiCamera:
    ...
    def autofocus(self, wait_sec=None):
        ...
        Older versions called autofocus_cycle(wait=True) and then waited wait_sec again.
        That made CAM2 feel like 2-3 seconds even when configured as 1.0 second.

        T16 behavior:
        - Start AF trigger/cycle without blocking when possible.
        - Wait only wait_sec total.
        - Do not add a second wait after AF.
        """
        if self.picam2 is None:
            raise RuntimeError(f"{self.name} not opened")
    ...
    def capture_file(self, path, return_to_preview=True):
        ...
        try:
            with self.camera_lock:
                # T16: capture directly from the already-running 2K stream.
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            ...
            current_camera = cam
            cam.start_preview()
            set_status(f"CAM1 Ready. No-switch 2K stream {PI_CAPTURE_WIDTH}x{PI_CAPTURE_HEIGHT}")
...
def capture_cam3_stage():
    ...
    try:
        ...
        if YOLO_PRELOAD_ENABLED and preloaded_yolo_model is not None:
            patch_yolo_loaders()
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Manual Capture - Dataset Mode T16 2K No-Switch Pi + YOLO Preload + AF Fix")
