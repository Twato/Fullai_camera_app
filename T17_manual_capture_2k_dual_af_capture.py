...
def _cached_camera_test_yolo(model_path, *args, **kwargs):
    ...
    if _original_camera_test_yolo is not None:
...
def preload_yolo_model_background():
    ...
    with yolo_preload_lock:
        ...
        try:
            ...
            if _original_camera_test_yolo is not None:
                preloaded_yolo_model = _original_camera_test_yolo(CAM3_YOLO_MODEL_PATH)
            else:
                from ultralytics import YOLO
                preloaded_yolo_model = YOLO(CAM3_YOLO_MODEL_PATH)

            # Patch camera_test.YOLO so UsbCameraTest.open() reuses this model instead of loading again.
            try:
...
class ManualPiCamera:
    ...
    def autofocus(self, wait_sec=None):
        """Trigger autofocus on IMX519/Pi autofocus cameras before capture."""
        if self.picam2 is None:
            raise RuntimeError(f"{self.name} not opened")
...
def capture_pi_stage():
    ...
    try:
        ...
        set_status(
            f"CAM1 + CAM2 autofocus together... CAM1 {PICAM1_FOCUS_DELAY_SEC:.1f}s / CAM2 {PICAM2_FOCUS_DELAY_SEC:.1f}s",
            BLUE,
        )
        ...
        th1.join()
        th2.join()
        print(f"CAM1+CAM2 parallel AF/capture total: {time.time() - t_all:.2f}s")
