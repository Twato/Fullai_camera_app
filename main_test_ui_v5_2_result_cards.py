...
def clear_result_cards():
    # Clear only the UI cards from the previous product/TOC review page.
    # This does not delete OCR data, images, or JSON files.
    for w in result_card_area.winfo_children():
        w.destroy()


def add_result_card(parent, title, image_data, fields, icon_key=None):
    card = tk.Frame(
        parent,
        bg=CARD,
    ...
    # ROI preview image
    roi_path = image_data.get("roi_path", "") if image_data else ""
    thumb = make_thumbnail_image(roi_path, size=(300, 135))
    ...
    if thumb is not None:
        ...
        img_box.config(text="NO IMAGE", fg="white", font=("Arial", 13, "bold"))
    ...
    for label_text, value_text in fields:
        ...
        box_bg = GREEN_BG if value_text and value_text != "(empty)" else "#f8f9fa"
        box_fg = GREEN if value_text and value_text != "(empty)" else "#7f8c8d"

        tk.Label(
            card,
            text=value_text or "(empty)",
            bg=box_bg,
            fg=box_fg,
            font=("Arial", 17, "bold"),
            anchor="w",
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    # CAM1 = Vendor + TOA image/result
    # CAM2 = TOB image/result
    add_result_card(
        result_card_area,
        "Vendor Label",
        cam1_data,
...
def show_toc_result(toc_items, fallback_data=None):
    ...
    if toc_items:
        # Show up to 3 TOC ROI cards for now.
        for item in toc_items[:3]:
            text = item.get("clean", "") or "(empty)"
            add_result_card(
                result_card_area,
                item.get("name", "TOC Label"),
                item,
                [
                    ("OCR Result", text),
                    ("RAW", item.get("raw", "")),
        ...
        text = fallback_data.get("clean", "") or "(empty)"
        add_result_card(
            result_card_area,
            "CAM3 Center ROI",
            fallback_data,
...
result_card_area = tk.Frame(page_result, bg=BG)
result_card_area.pack(fill="both", expand=True, padx=10, pady=(0, 12))

# Keep this hidden frame so older helper functions do not error.
# The new Product Result page uses result_card_area instead.
