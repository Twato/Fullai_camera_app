...
class ManualPiCamera:
    ...
    def capture_file(self, path):
        ...
        try:
            ...
            print(f"{self.name}: saved {path} at {frame.shape[1]}x{frame.shape[0]}")
            return path
        finally:
            self.capturing = False
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            ...
            current_camera = cam
            cam.start_preview()
            set_status(f"CAM2 Ready. Preview {PI_PREVIEW_WIDTH}x{PI_PREVIEW_HEIGHT} / Capture {PI_CAPTURE_WIDTH}x{PI_CAPTURE_HEIGHT}")
