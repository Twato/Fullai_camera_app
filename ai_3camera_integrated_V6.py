...
# Remove box / prepare next box
# V6: ใช้ threshold แบบ dynamic จากค่า object_score ตอน CAM0 เจอกล่อง
# ไม่ fix ค่าตายตัว เพราะแต่ละหน้างานแสง/มุมกล้องอาจไม่เท่ากัน
REMOVE_BOX_RATIO = 0.10           # กล่องออกเมื่อ score ต่ำกว่า 10% ของตอนเจอกล่อง
REMOVE_BOX_STABLE_TIME = 1.5      # ต้องต่ำต่อเนื่องกี่วินาที
REMOVE_BOX_CHECK_INTERVAL = 0.2
...
model = None
model_ready = False
last_picam0_background = None
last_picam0_object_score = None
...
def run_picamera(camera_index, camera_name):
    global current_camera, last_picam0_background, last_picam0_object_score
    ...
    try:
        ...
        # เก็บ background ล่าสุดของ CAM0 ไว้ใช้ตรวจว่าผู้ปฏิบัติงานเอากล่องออกแล้วหรือยัง
        if camera_index == PICAM0_INDEX:
            last_picam0_background = bg.copy()
        ...
        while running:
            ...
            if score > MOTION_THRESHOLD:
                # เก็บค่า score ตอนเจอกล่องจริงไว้ใช้เทียบตอนรอกล่องออก
                if camera_index == PICAM0_INDEX:
                    last_picam0_object_score = score

                set_status(f"{camera_name}: Object Detected / Score={score}")
                sleep(0.5)
...
# =========================================================
# REMOVE BOX / READY NEXT BOX
# =========================================================
def wait_remove_box_and_ready(inner_no, total_inner):
    """
    V6 Dynamic Remove Box Logic

    หลัง CAM0 + CAM1 ถ่ายครบ 1 กล่อง:
    - แจ้งให้เอากล่องออก
    - เปิด CAM0 เพื่อตรวจพื้นที่เดิม
    - เทียบ frame ปัจจุบันกับ background ว่างล่าสุดของ CAM0
    - ใช้ threshold แบบ dynamic จาก object_score ตอน CAM0 เจอกล่อง

    ตัวอย่างจาก log จริง:
    - ตอนมีกล่อง score ประมาณ 12,000,000 - 24,000,000
    - ตอนกล่องออก score ประมาณ 60,000 - 250,000
    ดังนั้น V6 ใช้ ratio เช่น 10% ของ object_score
    ถ้า score ลดลงต่ำกว่านี้ต่อเนื่อง REMOVE_BOX_STABLE_TIME = ถือว่ากล่องออกแล้ว
    """
    global current_camera
    ...
    try:
        show_popup(f"REMOVE BOX {inner_no}/{total_inner}", bg="#b36b00")
        set_status(f"Status: Inner Box {inner_no}/{total_inner} Completed / REMOVE BOX")
        progress_label.config(text=f"Remove Box: {inner_no} / {total_inner}")

        if last_picam0_background is None:
            set_status("Status: No CAM0 background reference / Wait 2 sec")
            sleep(2)
            show_popup("READY NEXT BOX", bg="#1f8f3a")
            return

        if last_picam0_object_score is None or last_picam0_object_score <= 0:
            set_status("Status: No CAM0 object score / Wait 2 sec")
            sleep(2)
            show_popup("READY NEXT BOX", bg="#1f8f3a")
            return

        remove_threshold = last_picam0_object_score * REMOVE_BOX_RATIO

        print("REMOVE BOX OBJECT SCORE:", last_picam0_object_score)
        print("REMOVE BOX DYNAMIC THRESHOLD:", int(remove_threshold))

        set_status(
            f"Status: Waiting Remove Box / "
            f"Threshold={int(remove_threshold)}"
        )

        cam = Picamera2(PICAM0_INDEX)
        current_camera = cam
        ...
        while running:
            ...
            gray = gray_from_frame(frame, is_bgr=False)
            score = motion_score(last_picam0_background, gray)

            ratio = score / max(last_picam0_object_score, 1)

            print(
                f"REMOVE BOX CHECK SCORE: {score} | "
                f"RATIO={ratio:.4f} | "
                f"THRESHOLD={int(remove_threshold)}"
            )
            ...
            if score < remove_threshold:
                ...
                stable_time = time() - stable_start

                set_status(
                    f"Status: Box Removed Stable "
                    f"{stable_time:.1f}/{REMOVE_BOX_STABLE_TIME:.1f} sec "
                    f"/ Ratio={ratio:.3f}"
                )

                if stable_time >= REMOVE_BOX_STABLE_TIME:
                    set_status("Status: READY NEXT BOX")
                    show_popup("READY NEXT BOX", bg="#1f8f3a")
                    sleep(1)
                    break
            else:
                ...
                set_status(
                    f"Status: Waiting Remove Box / "
                    f"Score={score} / Ratio={ratio:.3f}"
                )

            sleep(REMOVE_BOX_CHECK_INTERVAL)

    except Exception as e:
        print("wait_remove_box_and_ready Error:", e)
        set_status(f"Status: Remove Box Check Error {e}")
        sleep(1)
        ...
        try:
            ...
            print("Close remove-check camera error:", e)
...
def sequence_loop():
    ...
    while running and current_count < target_count:
        ...
        # รอให้ผู้ปฏิบัติงานเอากล่องออก แล้วค่อยพร้อมกล่องถัดไป
        # ทำทุกกล่อง รวมถึงกล่องสุดท้าย เพื่อให้พื้นที่หน้า CAM0 ว่างก่อนเข้า TOC
        if running:
            wait_remove_box_and_ready(current_count, target_count)
...
# =========================================================
root = tk.Tk()
root.title("AI Camera 3-Camera Data Collection V6")
...
title2 = tk.Label(
    page2,
    text="AI Camera 3-Camera Data Collection V6",
    font=("Arial", 16, "bold")
)
title2.pack(pady=5)
