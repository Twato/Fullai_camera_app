...
# =========================================================
# CAMERA READ FAIL GUARD
# For T27 Auto Camera Recovery:
# camera_test.py must stop waiting and return/raise when frame read fails
# continuously, so T27 can recover/re-scan USB device.
# =========================================================
MAX_READ_FAIL = 5
READ_FAIL_SLEEP = 0.2


def is_bad_frame(ret, frame):
    return (not ret) or frame is None


def raise_cannot_read_frame(camera_name, fail_count, extra=""):
    msg = f"{camera_name}: Cannot Read Frame x{fail_count}"
    if extra:
        msg += f" | {extra}"
...
def calc_motion(bg, frame):
    """Motion engine from Pi camera threshold test.

    Returns:
        gray, motion_score, max_area
    """
    ...
    return gray, score, int(max_area)
...
class PiCameraTest:
    ...
    def capture_preview_frame(self):
        if self.cam is None:
            raise RuntimeError(f"{self.name}: Cannot Read Frame | camera_not_open")
        try:
            ...
            raise RuntimeError(f"{self.name}: Cannot Read Frame | {e}")
    ...
    def wait_object(self, bg, running_check=lambda: True):
        """Wait until object is detected continuously for STABLE_TIME.

        Uses MOTION_THRESHOLD + MIN_AREA, so it will not capture immediately
        on the first motion spike.
        """
        ...
        while running_check():
            ...
            _, score, area = calc_motion(bg, frame)

            print(
                f"{self.name} MOTION SCORE: {score} | "
                f"AREA: {area} | "
            ...
            if score >= MOTION_THRESHOLD and area >= MIN_AREA:
                ...
                elapsed = time() - motion_start
                self.status(f"Object Detect {elapsed:.1f}/{STABLE_TIME:.1f}s | AREA={area}")
...
class UsbCameraTest:
    ...
    def open(self):
        ...
        while time() - start < 1.0:
            ...
            if is_bad_frame(ret, frame):
                fail_count += 1
                self.status(f"Cannot Read Frame {fail_count}/{MAX_READ_FAIL}")
                if fail_count >= MAX_READ_FAIL:
                    raise_cannot_read_frame(self.name, fail_count, f"/dev/video{self.device} warmup")
                sleep(0.1)
                continue
        ...
        if not read_ok_once:
            raise_cannot_read_frame(self.name, fail_count, f"/dev/video{self.device} no_frame_on_open")
    ...
    def capture_direct_or_yolo(self, path, running_check=lambda: True):
        if self.model is None:
            ...
            while running_check():
                ...
                if is_bad_frame(ret, frame):
                    ...
                    self.status(f"Cannot Read Frame {fail_count}/{MAX_READ_FAIL}")
                    if fail_count >= MAX_READ_FAIL:
                        raise_cannot_read_frame(self.name, fail_count, f"/dev/video{self.device} direct_mode")
                    sleep(READ_FAIL_SLEEP)
                    continue
        ...
        while running_check():
            ...
            if is_bad_frame(ret, frame):
                ...
                if fail_count >= MAX_READ_FAIL:
                    raise_cannot_read_frame(self.name, fail_count, f"/dev/video{self.device} yolo_mode")
