...
# =========================================================
root = tk.Tk()
root.title("AI Camera TEST UI - YOLO ROI OCR - T11 Compact Input UI")
...
# T11: keep the same card size as T10, but make all inner content compact enough
# to stay inside the card on Raspberry Pi / 1200x720 screens.
input_card = tk.Frame(
    page_input,
    bg=CARD,
    padx=42,
...
# Header is smaller than T10 so the utility rows never get pushed outside the card.
tk.Label(
    input_card,
    text="Start Test Job",
    font=("Arial", 27, "bold"),
...
def make_compact_input(parent, title):
    """Compact field row for scanner/manual input.
    Keeps label + input + check icon visually aligned while saving vertical space.
    """
...
# T11: compact validation area. Three validation lines stay short and never push buttons out.
input_status_frame = tk.Frame(input_card, bg=CARD)
input_status_frame.pack(fill="x", pady=(1, 6))

input_en_status = tk.Label(
    input_status_frame,
    text="EN : WAIT",
    bg=CARD,
    fg=MUTED,
...
input_delivery_status = tk.Label(
    input_status_frame,
    text="Delivery : WAIT",
    bg=CARD,
    fg=MUTED,
...
input_overall_status = tk.Label(
    input_status_frame,
    text="Input Not Ready",
    bg=CARD,
    fg=RED,
    font=("Arial", 10, "bold"),
    anchor="w",
)
input_overall_status.pack(fill="x", pady=(2, 0))
