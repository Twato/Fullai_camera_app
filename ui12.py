...
def add_toc_result_row(parent, row_no, title, image_data=None):
    """
    TOC result layout:
    7 fixed-like rows displayed vertically from TOC to TOC6.
    Left = ROI image, right = OCR information.
    """
    data = image_data or {}
    ...
    left_info = tk.Frame(row, bg=CARD, width=95)
    left_info.pack(side="left", fill="y", padx=(0, 10))
    ...
    # ROI image
    roi_path = data.get("roi_path", "")
    photo = load_result_photo(roi_path, size=(250, 58))
    ...
    value_box.pack(fill="x", pady=(3, 3))
...
def toc_loop():
    ...
    # 6 products require 1 TOC capture.
    ...
    for toc_index in range(toc_total):
        ...
        toc_no = toc_index + 1
        root.after(0, lambda n=toc_no, total=toc_total: set_progress(f"TOC {n} / {total}"))
...
def process_toc(toc_no=1, toc_total=1):
    ...
    root.after(0, lambda: set_step(f"CAM3 : TOC {toc_no}/{toc_total} YOLO detect -> capture -> crop ROI -> OCR"))
    root.after(0, lambda: set_status(f"CAM3 opening TOC {toc_no}/{toc_total}"))
    ...
    root.after(0, lambda: set_step(f"TOC {toc_no}/{toc_total} OCR : YOLO ROI count = {len(detections)}"))
    root.after(0, lambda: set_status(f"TOC {toc_no}/{toc_total} OCR reading from YOLO ROI"))
    ...
    # Wait for user to press OK / NEXT before next TOC capture or finish.
    wait_result_ok()
