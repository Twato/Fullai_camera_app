...
def init_job_data():
    ...
    now = datetime.now()
    job_storage_dir = ensure_dir(os.path.join(
        STORAGE_ROOT,
        now.strftime("%Y"),
        now.strftime("%m"),
        now.strftime("%d"),
        f"DELIVERY_{safe_folder_name(delivery_no)}",
    ))
...
def build_inner_box_record(inner_no, cam1_data, cam2_data):
    ...
    if picking_box_id:
        ...
        # No DB/API yet, so this is stored as WAIT information, not FAIL.
        pass
    ...
    inner_ref = f"INNER_{inner_no:03d}_VENDOR_{safe_folder_name(vendor_box_id)}"

    inner_folder = ensure_dir(os.path.join(job_storage_dir, "inner_boxes", inner_ref)) if job_storage_dir else ""
    vendor_folder = ensure_dir(os.path.join(inner_folder, vendor_folder_name)) if inner_folder else ""
    toa_folder = ensure_dir(os.path.join(inner_folder, toa_folder_name)) if inner_folder else ""
    tob_folder = ensure_dir(os.path.join(inner_folder, tob_folder_name)) if inner_folder else ""

    # Copy evidence into final storage. CAM1 currently represents Vendor + TOA. CAM2 represents TOB.
    evidence = {
        "vendor": {
...
def build_outer_box_record(toc_no, toc_total, item_map, fallback_data=None):
    ...
    for name, data in item_map.items():
        row_folder = ensure_dir(os.path.join(toc_folder, name.upper())) if toc_folder else ""
        evidence[name] = {
    ...
    if fallback_data is not None and "toc" not in evidence:
        row_folder = ensure_dir(os.path.join(toc_folder, "TOC_FALLBACK")) if toc_folder else ""
...
def finish_job_data():
    if not job_data:
        return
    job_data["job_finished_at"] = datetime.now().isoformat(timespec="seconds")
    update_job_summary()
    save_job_json()
...
def show_product_result(product_no, cam1_data, cam2_data):
    ...
    record = build_inner_box_record(product_no, cam1_data, cam2_data)
...
def show_toc_result(toc_items, fallback_data=None, toc_no=1, toc_total=1):
    ...
    record = build_outer_box_record(toc_no, toc_total, item_map, fallback_data=fallback_data)
