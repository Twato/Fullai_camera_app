...
def update_input_validation_ui(result=None):
    ...
    try:
        rows = [
            (input_en_status, result.get("en", {})),
            (input_delivery_status, result.get("delivery", {})),
            (input_qty_status, result.get("quantity", {})),
        ]
...
# =========================================================
root = tk.Tk()
root.title("AI Camera TEST UI - YOLO ROI OCR - T9 Input Safety")
...
input_card.place(relx=0.5, rely=0.50, anchor="center", width=820, height=680)
...
en_entry = make_input(input_card, "EN  (8 digits / scanner or manual)")
delivery_entry = make_input(input_card, "Delivary NO  (10 chars A-Z / 0-9)")
qty_entry = make_input(input_card, "Quantity  (1-1000)")

# T9 input validation status
input_status_frame = tk.Frame(input_card, bg=CARD)
input_status_frame.pack(fill="x", pady=(0, 8))
...
camera_precheck_status = tk.Label(input_card, text="Camera Pre-check : WAIT", bg="#f7f9fa", fg=MUTED, font=("Consolas", 9), justify="left", anchor="w", padx=8, pady=5)
camera_precheck_status.pack(fill="x", pady=(0, 8))
...
util_btn_row.pack(fill="x", pady=(0, 8))
tk.Button(util_btn_row, text="SCANNER TEST", font=("Arial", 12, "bold"), bg="#dfe6e9", fg=TEXT, activebackground="#cfd8dc", activeforeground=TEXT, relief="flat", command=open_scanner_test).pack(side="left", fill="x", expand=True, ipady=5, padx=(0, 5))
tk.Button(util_btn_row, text="CAMERA PRE-CHECK", font=("Arial", 12, "bold"), bg="#dfe6e9", fg=TEXT, activebackground="#cfd8dc", activeforeground=TEXT, relief="flat", command=run_camera_precheck_button).pack(side="left", fill="x", expand=True, ipady=5, padx=(5, 0))

tk.Button(input_card, text="START TEST", font=("Arial", 20, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job).pack(fill="x", ipady=8, pady=(4, 8))
tk.Button(input_card, text="EXIT APP", font=("Arial", 20, "bold"), bg=RED, fg="white", activebackground=RED_DARK, activeforeground="white", relief="flat", command=exit_app).pack(fill="x", ipady=8)
