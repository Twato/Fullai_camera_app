...
# =========================================================
# STORAGE RETENTION CLEANUP (T20)
# Auto-delete delivery data older than 90 days and keep cleanup logs.
# It deletes only job folders under storage/YYYY/MM/DD/ and never deletes
# global_search_index.json or cleanup log files.
# =========================================================
RETENTION_DAYS = 90
CLEANUP_LOG_DIR = os.path.join(STORAGE_ROOT, "cleanup_logs")


def cleanup_empty_date_dirs():
    """Remove empty YYYY/MM/DD, YYYY/MM, and YYYY folders after cleanup."""
    try:
        # Remove empty DD folders first.
        for year_name in sorted(os.listdir(STORAGE_ROOT)):
            year_path = os.path.join(STORAGE_ROOT, year_name)
            if not (os.path.isdir(year_path) and year_name.isdigit() and len(year_name) == 4):
                continue
            for month_name in sorted(os.listdir(year_path)):
                month_path = os.path.join(year_path, month_name)
                if not (os.path.isdir(month_path) and month_name.isdigit() and len(month_name) == 2):
                    continue
                for day_name in sorted(os.listdir(month_path)):
                    day_path = os.path.join(month_path, day_name)
                    if os.path.isdir(day_path) and day_name.isdigit() and len(day_name) == 2:
                        try:
...
def auto_delete_old_storage(retention_days=RETENTION_DAYS):
    """
    Delete completed/cancelled/failed job delivery folders older than retention_days.

    Expected storage structure:
        storage/YYYY/MM/DD/DELIVERY_xxx/...
    ...
    started_at = now_iso()
    now_dt = datetime.now()
    cutoff_dt = now_dt - timedelta(days=int(retention_days)) if 'timedelta' in globals() else None

    # Fallback because the original T8 imported only datetime.
    cutoff_timestamp = time.time() - (int(retention_days) * 24 * 60 * 60)
    deleted_items = []
    skipped_items = []
    error_items = []

    print("========== STORAGE CLEANUP T20 ==========")
    ...
    try:
        for year_name in sorted(os.listdir(STORAGE_ROOT)):
            ...
            for month_name in sorted(os.listdir(year_path)):
                ...
                for day_name in sorted(os.listdir(month_path)):
                    day_path = os.path.join(month_path, day_name)
                    if not (os.path.isdir(day_path) and day_name.isdigit() and len(day_name) == 2):
                        continue

                    # Prefer folder date from YYYY/MM/DD. If invalid, use mtime fallback.
                    is_old = False
                    folder_date_text = f"{year_name}-{month_name}-{day_name}"
                    try:
                        folder_dt = datetime.strptime(folder_date_text, "%Y-%m-%d")
                        if cutoff_dt:
                            is_old = folder_dt.date() < cutoff_dt.date()
                        else:
                            is_old = os.path.getmtime(day_path) < cutoff_timestamp
                    except Exception:
                        is_old = os.path.getmtime(day_path) < cutoff_timestamp
                    ...
                    if not is_old:
                        skipped_items.append({
                            "type": "date_folder",
                            "path": rel_storage_path(day_path),
                            "reason": "within_retention",
                        })
                        continue

                    # Delete only delivery/job folders inside this old day folder.
                    for job_name in sorted(os.listdir(day_path)):
                        ...
                        try:
                            ...
                            deleted_items.append({
                                "type": "job_folder",
                                "date": folder_date_text,
                                "path": rel_storage_path(job_path),
                            })
                            print("Deleted old job:", rel_storage_path(job_path))
                        except Exception as e:
                            error_items.append({
                                "type": "job_folder",
                                "date": folder_date_text,
                            ...
                            print("Delete old job error:", job_path, e)

        cleanup_empty_date_dirs()

    except Exception as e:
        error_items.append({"type": "cleanup_process", "path": rel_storage_path(STORAGE_ROOT), "error": str(e)})
        print("Storage cleanup error:", e)

    finished_at = now_iso()
    log_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "cleanup_version": "T20_AUTO_DELETE_90_DAYS",
        "retention_days": retention_days,
        "started_at": started_at,
        "finished_at": finished_at,
        "duration_sec": seconds_between(started_at, finished_at),
        "storage_root": STORAGE_ROOT,
        "deleted_count": len(deleted_items),
        "skipped_date_folder_count": len(skipped_items),
    ...
    txt_log_path = os.path.join(CLEANUP_LOG_DIR, f"cleanup_{log_stamp}.txt")
    json_log_path = os.path.join(CLEANUP_LOG_DIR, f"cleanup_{log_stamp}.json")

    try:
        with open(txt_log_path, "w", encoding="utf-8") as f:
            f.write("STORAGE CLEANUP LOG\n")
            f.write("===================\n")
            f.write(f"Version        : T20_AUTO_DELETE_90_DAYS\n")
            f.write(f"Started At     : {started_at}\n")
            f.write(f"Finished At    : {finished_at}\n")
            f.write(f"Duration Sec   : {log_data['duration_sec']}\n")
            f.write(f"Retention Days : {retention_days}\n")
            f.write(f"Storage Root   : {STORAGE_ROOT}\n")
            f.write(f"Deleted Count  : {len(deleted_items)}\n")
            f.write(f"Error Count    : {len(error_items)}\n\n")

            if deleted_items:
                f.write("Deleted Items:\n")
                for item in deleted_items:
                    f.write(f"- {item.get('date', '')} | {item.get('path', '')}\n")
            else:
                f.write("Deleted Items: none\n")

            if error_items:
                f.write("\nErrors:\n")
                for item in error_items:
                    f.write(f"- {item.get('path', '')} | {item.get('error', '')}\n")

        write_json_file(json_log_path, log_data)
        print("Saved cleanup log:", txt_log_path)
        print("Saved cleanup JSON:", json_log_path)
    except Exception as e:
        print("Save cleanup log error:", e)

    print(f"Storage cleanup finished. Deleted={len(deleted_items)} Error={len(error_items)}")
    print("=========================================")
    return log_data
...
# =========================================================
root = tk.Tk()
root.title("AI Camera TEST UI - YOLO ROI OCR - T20 Auto Delete 90 Days")
...
tk.Button(input_card, text="START TEST", font=("Arial", 21, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job).pack(fill="x", ipady=10, pady=(4, 8))
...
# T20: run 90-day storage retention cleanup once when the application starts.
# Rebuild the search index afterward so old deleted jobs are removed from global_search_index.json.
try:
    auto_delete_old_storage(RETENTION_DAYS)
    rebuild_global_search_index()
except Exception as e:
    print("T20 startup cleanup error:", e)
