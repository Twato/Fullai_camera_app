...
def auto_delete_old_storage(retention_days=None):
    ...
    log_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "version": "T22_RETENTION_DAILY_CLEANUP_CAMERA_RESET",
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T22_RETENTION_DAILY_CLEANUP_CAMERA_RESET",
...
def reset_current_camera():
    """T22: Reset only the currently active camera/stage without cancelling the job.

    This closes the current camera handle. The active capture loop will stop,
    mark the current process attempt as reset, and retry the same Inner/Outer index.
    """
    ...
    try:
        set_status("Reset current camera... closing camera and retrying same step")
    except Exception:
        pass
...
def make_validation_status_badge(parent, status):
    status = (status or "WAIT").upper()
    if status == "PASS":
        text, bg, fg = "โ“ PASS", GREEN, "white"
    elif status == "NG":
        text, bg, fg = "โ— NG", RED, "white"
    else:
...
def schedule_storage_cleanup():
    ...
    def run_cleanup_worker():
        ...
        try:
            print("=" * 60)
            print("T22 DAILY STORAGE CLEANUP START", now_iso())
            deleted = auto_delete_old_storage()
            print(f"T22 DAILY STORAGE CLEANUP DONE. Deleted {len(deleted)} day folder(s).")
            print("=" * 60)
            try:
            ...
            print("T22 DAILY STORAGE CLEANUP ERROR:", e)
            try:
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Station - YOLO ROI OCR - T22")
