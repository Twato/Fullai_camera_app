...
def set_checklist(active=None, done=None, mode="inner"):
    idx, total = get_current_count("outer" if mode == "outer" else "inner")
    count_text = f"{idx}/{total}" if total else "-/-"
    done = done or []
...
input_card = tk.Frame(page_input, bg=CARD, padx=42, pady=34, highlightbackground=BORDER, highlightthickness=1)
input_card.place(relx=0.5, rely=0.48, anchor="center", width=560, height=470)
...
flat_button(header_buttons, "Restart The Process", bg="#bfc3c7", fg=TEXT, command=restart_process, icon_key="refresh", width=20).pack(side="left", padx=(0, 8))
flat_button(header_buttons, "Cancel Job", bg=RED, fg="white", command=cancel_job, icon_key="cancel", width=14).pack(side="left", padx=(0, 8))
flat_button(header_buttons, "OK / NEXT", bg="#27ae60", fg="white", command=on_result_ok, width=14).pack(side="left")
...
info_delivery = header_info_box(info_row1, "Delivary No", 18)
info_item = header_info_box(info_row1, "Item No", 14)
...
ok_next_btn = tk.Button(result_button_frame, text="OK / NEXT", bg="#27ae60", fg="white", activebackground="#1e8449", activeforeground="white", font=("Arial", 12, "bold"), relief="flat", cursor="hand2", padx=18, pady=8, command=on_result_ok)
ok_next_btn.pack(side="right", padx=(8, 0))

restart_result_btn = tk.Button(result_button_frame, text="Restart The Process", bg="#bfc3c7", fg=TEXT, activebackground="#aeb4b8", activeforeground=TEXT, font=("Arial", 12, "bold"), relief="flat", cursor="hand2", padx=14, pady=8, command=restart_process)
restart_result_btn.pack(side="right", padx=(8, 0))

cancel_result_btn = tk.Button(result_button_frame, text="Cancel Job", bg=RED, fg="white", activebackground=RED_DARK, activeforeground="white", font=("Arial", 12, "bold"), relief="flat", cursor="hand2", padx=14, pady=8, command=cancel_job)
cancel_result_btn.pack(side="right", padx=(8, 0))
