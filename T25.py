...
# =========================================================
APP_VERSION = "T25_AUTO_CAMERA_RECOVERY"
...
try:
    ...
    print("T25 Pi camera threshold applied:",
...
def auto_delete_old_storage(retention_days=None):
    ...
    log_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "version": "T25_AUTO_CAMERA_RECOVERY",
...
def init_job_data():
    ...
    job_data = {
        "project": "AI-Based Validation System for Shipping Label Verification",
        "design_version": "T25_AUTO_CAMERA_RECOVERY",
    ...
    while running:
            ...
            try:
                ...
                if bg is None:
                    ...
                    if cam1_retry < AUTO_CAMERA_RECOVERY_MAX_RETRY and auto_recover_camera("CAM1", "background_not_ready"):
                        cam1_retry += 1
                        continue
                    root.after(0, lambda: messagebox.showerror("CAM1 Error", "CAM1 cannot learn background. Please check camera and press Reset Camera."))
                    return False
                ...
                if not detected:
                    ...
                    if cam1_retry < AUTO_CAMERA_RECOVERY_MAX_RETRY and auto_recover_camera("CAM1", "object_detect_failed"):
                        cam1_retry += 1
                        continue
                    root.after(0, lambda: messagebox.showerror("CAM1 Error", "CAM1 cannot detect/read frame. Please check camera and press Reset Camera."))
                    return False
                ...
                root.after(0, lambda e=e: messagebox.showerror("CAM1 Error", f"{e}\n\nAuto recovery failed. Please check camera cable and press Reset Camera."))
                return False
        ...
        while running:
            ...
            try:
                ...
                root.after(0, lambda e=e: messagebox.showerror("CAM2 Error", f"{e}\n\nAuto recovery failed. Please check camera cable and press Reset Camera."))
                return False
...
def process_toc(toc_no=1, toc_total=1):
    ...
    while running:
        ...
        while running:
            ...
            try:
                log_event("CAMERA_OPEN_START", camera="CAM3", device=USB_DEVICE, retry=cam3_retry)
                cam3.open()
                log_event("CAMERA_OPEN_OK", camera="CAM3", device=USB_DEVICE, retry=cam3_retry)
                ...
                if not capture_result:
                    ...
                    root.after(0, lambda: messagebox.showerror("CAM3 Error", "CAM3 cannot read frame / no capture result.\n\nPlease unplug/replug USB camera and press Reset Camera."))
                    return False
                ...
                root.after(0, lambda e=e: messagebox.showerror("CAM3 Error", f"{e}\n\nAuto recovery failed. Please unplug/replug USB camera and press Reset Camera."))
                return False
...
def schedule_storage_cleanup():
    ...
    def run_cleanup_worker():
        ...
        try:
            print("=" * 60)
            print("T25 DAILY STORAGE CLEANUP START", now_iso())
            deleted = auto_delete_old_storage()
            print(f"T25 DAILY STORAGE CLEANUP DONE. Deleted {len(deleted)} day folder(s).")
            print("=" * 60)
            try:
            ...
            print("T25 DAILY STORAGE CLEANUP ERROR:", e)
            try:
...
root.title("AI Camera Station - YOLO ROI OCR - T25")
