...
def make_product_result_section(parent, key, title, icon_key, field1, field2):
    ...
    first = make_result_field(card, field1)
    second = make_result_field(card, field2)
...
def add_toc_result_card(parent, title, image_data, fields, icon_key=None):
    """
    Temporary card for TOC result only.
    Product result page uses fixed widgets and does not call this.
    """
    card = tk.Frame(
        parent,
        bg=CARD,
    ...
    img_box = tk.Label(card, bg=DARK, width=300, height=135)
