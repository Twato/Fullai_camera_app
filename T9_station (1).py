...
def auto_delete_old_storage(retention_days=90):
    """Delete YYYY/MM/DD storage folders older than retention_days and rebuild the audit index."""
    ensure_dir(STORAGE_ROOT)
    cutoff = datetime.now() - timedelta(days=int(retention_days))
    deleted = []

    try:
        for year_name in os.listdir(STORAGE_ROOT):
...
def startup_preload_worker():
    ...
    try:
        ...
        set_preload_status(detail=f"Auto-delete done. Deleted {len(deleted)} old day folder(s).", cleanup="OK")
    except Exception as e:
...
# =========================================================
root = tk.Tk()
root.title("AI Camera Station - YOLO ROI OCR - T9")
