...
# =========================================================
# T9 PI CAMERA MOTION DETECTION CONFIG
# =========================================================
...
DIFF_THRESHOLD = 30
BLUR_SIZE = 21
MOTION_PRINT_INTERVAL = 0.15
BACKGROUND_WAIT_SEC = 1.2
...
class ManualPiCamera:
    """
    T9: Pi Camera wrapper with Motion Detection auto capture.
    - ใช้ Picamera2 สำหรับ CAM1/CAM2
    - Preview ยังแสดงตลอดเหมือนเดิม
    - Capture จะเกิดเมื่อ motion score + area ผ่าน threshold ต่อเนื่องครบ STABLE_TIME
    """
    ...
    def calc_motion(self, background_gray, frame):
        ...
        return gray, motion_score, int(max_area), max_box

    def wait_background(self):
        set_status(f"{self.name} building background... keep camera clear")
        end_time = time.time() + BACKGROUND_WAIT_SEC
        frame = None
        while time.time() < end_time:
            if preview_stop.is_set() or self.picam2 is None or not running:
                return None
            frame = self.get_frame()
            time.sleep(0.05)
        if frame is None:
            frame = self.get_frame()
        self.background_gray = self.make_gray(frame)
        set_status(f"{self.name} Background Ready", GREEN)
        return self.background_gray
    ...
    def wait_object(self):
        ...
        motion_start = None
        last_print = 0
        set_status(f"{self.name} Waiting Object")

        while not preview_stop.is_set() and self.picam2 is not None and running:
            frame = self.get_frame()
            _, score, area, box = self.calc_motion(self.background_gray, frame)
            now = time.time()

            if now - last_print >= MOTION_PRINT_INTERVAL:
                print(f"{self.name} MOTION SCORE: {score} | AREA: {area}")
                last_print = now
            ...
            if score >= MOTION_THRESHOLD and area >= MIN_AREA:
                ...
                set_status(f"{self.name} Detect {elapsed:.1f}/{STABLE_TIME:.1f}s | Score={score} Area={area}", BLUE)
                if elapsed >= STABLE_TIME:
                    print("=" * 50)
                    print(f"{self.name} OBJECT DETECTED")
                    print(f"SCORE = {score}")
                    print(f"AREA  = {area}")
    ...
    def motion_capture_file(self, path):
        bg = self.wait_background()
        if bg is None:
            return False
        ok = self.wait_object()
        if not ok:
            return False
        set_status(f"{self.name} autofocus {FOCUS_DELAY_SEC:.1f}s...")
        self.autofocus(FOCUS_DELAY_SEC)
        set_status(f"{self.name} capturing...")
        self.capture_file(path)
        set_status(f"Saved {self.name}: {path}", GREEN)
        return True
...
def open_current_stage_camera():
    ...
    try:
        if current_stage == "cam1":
            set_step(f"Inner Box {inner_index}/{quantity} : CAM1 auto motion detect -> autofocus -> capture")
            set_status("CAM1 opening...")
            cam = ManualPiCamera(PICAM1_INDEX, "CAM1")
            cam.open()
            current_camera = cam
            cam.start_preview()
            set_status("CAM1 Ready. Auto motion detection will start")
            root.after(800, lambda: capture_current() if running and current_stage == "cam1" else None)

        elif current_stage == "cam2":
            set_step(f"Inner Box {inner_index}/{quantity} : CAM2 auto motion detect -> autofocus -> capture")
            set_status("CAM2 opening...")
            cam = ManualPiCamera(PICAM2_INDEX, "CAM2")
            cam.open()
            current_camera = cam
            cam.start_preview()
            set_status("CAM2 Ready. Auto motion detection running")
...
def reset_current_camera():
    ...
    time.sleep(0.8)  # เนเธซเน OS เธชเธฃเนเธฒเธ /dev/video เนเธซเธกเนเธซเธฅเธฑเธเธ–เธญเธ”เน€เธชเธตเธขเธ
...
def capture_pi_stage():
    """
    T9 Flow:
    - CAM1 รอ motion detection แล้วถ่ายอัตโนมัติ
    - หลัง CAM1 เสร็จ เปิด CAM2 แล้วรอ motion detection ถ่ายอัตโนมัติ
    - ครบ CAM1/CAM2 แล้วไป Inner Box ถัดไป หรือไป CAM3
    """
    global current_stage, inner_index, running, auto_cam3_capture_on_ready, current_camera, last_preview_time

    if current_stage != "cam1":
        set_status("CAM1/CAM2 are auto capture. Please wait for the current process.", YELLOW)
        return
    ...
    # ===================== CAM1 AUTO MOTION CAPTURE =====================
    try:
        if current_camera is None:
            raise RuntimeError("CAM1 is not opened")
        set_status("CAM1 auto motion detection running...")
        ok = current_camera.motion_capture_file(cam1_path)
        if not ok:
            raise RuntimeError("CAM1 motion capture stopped or failed")
    except Exception as e:
    ...
    # ===================== CAM2 AUTO MOTION CAPTURE =====================
    cam2 = None
    try:
        current_stage = "cam2"
        root.after(0, update_header)
        root.after(0, lambda: set_step(f"Inner Box {inner_index}/{quantity} : CAM2 auto motion detect after CAM1"))
        set_status("CAM2 opening for auto motion capture...")
        ...
        set_status("CAM2 auto motion detection running...")
        ok = cam2.motion_capture_file(cam2_path)
        if not ok:
            raise RuntimeError("CAM2 motion capture stopped or failed")
    except Exception as e:
...
def show_complete():
    complete_title.config(text="T9 STATION COMPLETED")
...
# =========================================================
root = tk.Tk()
root.title("AI Camera T9 Station - Motion Auto Capture")
...
tk.Label(input_card, text="T9 Station Auto Capture", font=("Arial", 22, "bold"), bg=CARD, fg=TEXT).pack(anchor="w", pady=(0, 10))
...
tk.Button(input_card, text="START T9 STATION", font=("Arial", 15, "bold"), bg=BLUE, fg="white", activebackground=BLUE_DARK, activeforeground="white", relief="flat", command=start_job).pack(fill="x", ipady=8, pady=(2, 8))
...
complete_title = tk.Label(complete_card, text="T9 STATION COMPLETED", font=("Arial", 15, "bold"), bg=CARD, fg=GREEN)
